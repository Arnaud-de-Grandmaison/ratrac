...
public:
  ...
  Canvas(const Canvas &Other) : m_width(Other.m_width), m_height(Other.m_height),
          m_canvas(Other.m_canvas) {
  }
  Canvas(Canvas &&Other) : m_width(Other.m_width), m_height(Other.m_height), m_canvas(std::move(Other.m_canvas)) {
    Other.m_width =0;
    Other.m_height = 0;
  }
  ...
  const Color &at(unsigned x, unsigned y) const { return m_canvas[y * m_width + x]; }
