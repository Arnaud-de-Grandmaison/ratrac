...
TEST(RayTracerMatrice, other_stuff) {
  ...
  EXPECT_EQ(submatrix(M, 2, 1), Matrice(std::vector<std::vector<float>>(
                                    {{-6, 1, 6}, {-8, 8, 6}, {-7, -1, 1}})));

  // Calculating a minor of a 3*3 matrix
  M = std::vector<std::vector<float>>({{3, 5, 0}, {2, -1, -7}, {6, -1, 5}});
  ...
  M = std::vector<std::vector<float>>({{1, 2, 6}, {-5, 8, -4}, {2, 6, 4}});
  ...
  EXPECT_TRUE(M2.approximatly_equal(Matrice(std::vector<std::vector<float>>(
  ...
  EXPECT_TRUE(
      inverse(M).approximatly_equal(Matrice(std::vector<std::vector<float>>(
  ...
  M2 = std::vector<std::vector<float>>(
