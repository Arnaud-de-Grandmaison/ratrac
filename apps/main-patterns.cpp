...
int main(int argc, char *argv[]) {
  ...
  middle->transform(Matrice::translation(-1.0, 1.0, 2.0) *
                    Matrice::rotation_y(-M_PI / 2));
  Material m2;
  m2.pattern(Stripes(Color(0.1, 0.8, 0.1), Color(0.5, 0.8, 0.5),
                     Matrice::scaling(0.15, 0.15, 0.15) *
                         Matrice::rotation_z(-M_PI / 8) *
                         Matrice::rotation_y(-M_PI / 3)));
  m2.diffuse(0.7);
  m2.specular(0.3);
  middle->material(m2);
  world.append(middle);
